...
namespace CGL {
    ...
    Ray Camera::generate_ray(double x, double y) const {
        ...
        //
        double cameraX = 0.5 * tan(radians(hFov));
        double cameraY = 0.5 * tan(radians(vFov));

        double X = -cameraX + (cameraX + cameraX) * x;
        double Y = -cameraY + (cameraY + cameraY) * y;
        Vector3D dir = c2w * Vector3D(X, Y, -1);
        dir.normalize();
        Ray result = Ray(pos, dir, fClip);
        result.min_t = nClip;
        return result;
